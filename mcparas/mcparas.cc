...
int mcparas::propagation_set_whole(prop_mode prop) {
...
int mcparas::propagation_set(prop_mode prop) {
  double *point = p + p_index;
  unsigned tmpind = 0;
...
int mcparas::proton_set(inject_mode bks) {
  double *point = p + p_index;
...
int mcparas::electron_set(inject_mode bks) {
  double *point = p + p_index;
...
int mcparas::branch_set(anaspec::branch_choice branch) const {
  if(branch >= (int)anaspec::branch_num) {
...
int mcparas::branch_set(double *branches) const {
  if (galdef->DM_int0 == pulsar) return 1; // Don't initial the branch in the pulsar case;
...
int mcparas::branch_set(mix_branch mixbran) {
  ...
  switch(mixbran) {
  case emutau:
    if(1 - point[0] - point[1] < -1e-7) return 1;
  ...
  case emutaub:
    if(1 - point[0] - point[1] - point[2] < -1e-7) return 1;
    branches[anaspec::electron] = point[0];
    branches[anaspec::mu] = point[1];
