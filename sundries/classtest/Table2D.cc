...
int Table2D::insline(double x_) {
  LineConsIter xiter = xaxis.find(x_);

  if (xiter != xaxis.end()) return 0;
...
int Table2D::inscolm(double y_) {
  LineConsIter yiter = yaxis.find(y_);

  if (yiter != yaxis.end()) return 0;
...
double Table2D::dx2(TabConsIter rowmiter, LineConsIter yiter) const {
  ...
  return (val3 * (x2 - x1) + val2 * (x1 - x3) + val1 * (x3 - x2)) * 2
         / ((x3 - x1) * (x2 - x1) * (x3 - x2));
}

double Table2D::dx2(TabConsIter xiter) const {
  LineConsIter yiter =  yaxis.begin();
  double dx2_max = 0,
         dx2tmp;

  for (; yiter != yaxis.end(); ++yiter) {
